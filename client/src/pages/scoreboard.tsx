        ...
        {topThree.length > 0 && (
          <div className="flex justify-center items-end mb-8 space-x-4">
            {podiumOrder.map((player, podiumIndex) => {
              ...
              const actualPlace = sortedPlayers.findIndex(p => p.id === player.id) + 1;
              const placeText = actualPlace === 1 ? '1st' : actualPlace === 2 ? '2nd' : '3rd';
              
              return (
                <div key={player.id} className="text-center">
                  <div className={`${getPodiumColors(actualPlace)} text-white ${getPodiumHeight(actualPlace)} rounded-t-3xl flex flex-col items-center justify-center mb-2`}>
                    <div className="text-2xl mb-1">{player.avatar}</div>
                    <div className="text-xs font-bold">{placeText}</div>
                  </div>
                  <div className={`bg-white rounded-2xl p-4 shadow-xl ${actualPlace === 1 ? 'border-4 border-quiz-yellow' : ''}`}>
                    <div className="font-bold text-gray-800">{player.name}</div>
                    <div className={`text-2xl font-bold ${actualPlace === 1 ? 'text-quiz-yellow' : 'text-quiz-blue'}`}>
          ...
          <div className="bg-white bg-opacity-20 backdrop-blur-sm rounded-3xl p-6 mb-8">
            <div className="space-y-3">
              {remaining.map((player, index) => {
                ...
                return (
                  <div key={player.id} className="flex items-center justify-between bg-white bg-opacity-20 rounded-xl p-4">
